...
import { Passcode } from '@/types/database';
import crypto from 'crypto';

export default function EditPasscode({ params }: { params: { id: string } }) {
  ...
  useEffect(() => {
    ...
    const fetchPasscode = async () => {
      try {
        ...
        if (!data) throw new Error('Passcode not found');

        // Decrypt the passcode
        const decryptedPasscode = decryptData(data.passcode, user.id);
        ...
        setError(err.message || 'Failed to fetch passcode');
        setIsLoading(false);
      }
    };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!user) {
      setError('You must be logged in to update a passcode');
      return;
    }
  ...
  return (
    <div className="py-10">
      <div className="max-w-3xl mx-auto">
        <h1 className="text-2xl font-bold text-gray-900">Edit Device Passcode</h1>
        <p className="mt-2 text-sm text-gray-600">
          Update your device passcode that will be sent if you're marked as deceased.
        </p>
        ...
        <form onSubmit={handleSubmit} className="mt-6 space-y-6 bg-white shadow px-4 py-5 sm:rounded-lg sm:p-6">
          ...
          <div className="flex justify-end">
            ...
            >
              {isSubmitting ? 'Saving...' : 'Update Passcode'}
            </button>
          </div>
        </form>
      </div>
